...
void add_food_order(int category, int type) {
    ...
    int idx = order_count++;
    orders[idx].category = category;
    orders[idx].type = type;
    orders[idx].collected = 0;
    orders[idx].ordered_at = time(NULL);
    if (category == 1) {
        orders[idx].duration_seconds = 3 * 60;
        if (type == 1) {
            strcpy(orders[idx].label, "Bakso Original");
        } else if (type == 2) {
            strcpy(orders[idx].label, "Bakso Pangsit");
        } else {
            strcpy(orders[idx].label, "Bakso Mercon");
        }
    } else if (category == 2) {
        orders[idx].duration_seconds = 1 * 60;
        if (type == 1) {
            strcpy(orders[idx].label, "Soto Ayam");
        } else {
            strcpy(orders[idx].label, "Soto Sapi");
        }
    } else {
        orders[idx].duration_seconds = 2 * 60;
        if (type == 1) {
            strcpy(orders[idx].label, "Mie Ayam Original");
        } else if (type == 2) {
            strcpy(orders[idx].label, "Mie Ayam Jamur");
        } else {
            strcpy(orders[idx].label, "Mie Ayam Tanpa Ayam");
        }
    }
    orders[idx].ready_at = orders[idx].ordered_at + orders[idx].duration_seconds;
...
int play_minesweeper(void) {
    ...
    printf("Pilih level Minesweeper: 1=easy, 2=hard\n");
    int level;
    if (scanf("%d", &level) != 1) {
        clear_input_buffer();
    ...
    if (level == 1) {
        game.rows = 10;
        game.cols = 10;
        filename = "level_easy.txt";
    } else if (level == 2) {
        game.rows = 15;
        game.cols = 15;
        filename = "level_hard.txt";
    } else {
